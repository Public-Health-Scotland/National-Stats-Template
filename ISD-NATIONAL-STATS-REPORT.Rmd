...
for (i in 1:length(file_list)){
  ...
  paths[i] <- file_list[i]  # vector with filepaths
  sizeskb[i] <- round(file.size(paths[i]) / 1024) # vector with filesize in kb  
  text <- unlist(strsplit(file_list[i], ".", fixed = TRUE)) # split filename in name and extension
  file_name[i] <- text[1] # vector with file names
  
  # if file extension is XLS or XLSX rename to Excel, else keep extension as is 
  if((!is.na(text[2])) & toupper(substring(text[2], 1, 3)) == "XLS") { 
