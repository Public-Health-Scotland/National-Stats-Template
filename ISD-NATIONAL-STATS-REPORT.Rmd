...
#Figure1_filename <- paste0(output_report_filepath, "/Figure1_ggplot.png") 

ggsave("Figure1_ggplot.png", plot = FigureOne, width = 17.78, height = 14.24, units = "cm", 
       device = "png", dpi = 600)
...
#Figure2_filename <- paste0(output_report_filepath, "/Figure2_ggplot.png")
...
#file_list <- list.files(output_datafile_filepath)#
file_list <- list.files()#test this to see if it works!
paths <- c()
sizeskb <- c()
file_name <- c()
file_size <- c()
...
for (i in 1:length(file_list)){
  #paths[i] <- paste0(output_datafile_filepath,"/", file_list[i])
  paths[i] <- file_list[i] #test this to see if it works!
  sizeskb[i] <- round(file.size(paths[i]) / 1024)
  text <- unlist(strsplit(file_list[i], ".", fixed = TRUE))
  file_name[i] <- text[1]
  #if(toupper(substring(text[2], 1, 3)) == "XLS") {
